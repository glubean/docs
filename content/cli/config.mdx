...
`contract`, and `flow` suites for `glubean run` / `glubean ci run`.
Load plans are selected by `glubean load [target]` instead of profiles.
...
files, then run them with `glubean load [file|dir|glob]`. `glubean.yaml`
profiles do not select load plans in the current CLI.
