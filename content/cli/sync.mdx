...
<AskAgent
  prompts={[
    {
      intent: "Sync now",
      text: "Run glubean sync for this repo and confirm the Cloud Specifications now match the current contracts and workflows.",
    },
    {
      intent: "Sync or upload?",
      text: "I just changed «contracts / test logic / load plans». Decide whether that calls for sync, run --upload, or both — and do it.",
    },
  ]}
/>
...
```bash
npm run doctor    # read-only readiness check
npm run sync      # generated command: glubean sync --env-file .env
```
