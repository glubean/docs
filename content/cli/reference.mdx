...
- `--env-file <path>`: Load environment variables from a specific file. Defaults to `.env` (which also auto-loads `.env.secrets`). For resolution order, system fallback, and `${...}` mapping, see [Environments & Secrets](/guides/environments).
