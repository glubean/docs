...
The extension keeps per-test history for local editor runs under:
...
Full CLI or package-script runs also write the latest project result to
`.glubean/last-run.result.json`. Open that file directly, or use
`Glubean: Open Latest Result`, when you want to inspect the most recent
project-level run rather than a single test's local history.
