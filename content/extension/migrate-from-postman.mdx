...
The AI assistant needs to see `deno.json`, the `@glubean/sdk` import map, and `tests/` conventions. Without this context, it will generate Node/Jest-style code instead of Glubean SDK tests.
</Callout>
...
Make sure you are in a Glubean project root (created by `glubean init`).
...
```text
You are in a Glubean project (Deno + @glubean/sdk).
...
- [Quick Start](/extension/quick-start) — install the extension and run `glubean init`
- [AI Integration](/extension/ai-integration) — use `glubean context` for even better AI output
- [Workflow: Explore vs Tests](/extension/workflow) — start in `explore/`, promote to `tests/`
