...
Place the relevant files in the repo:
...
- `.http` files
- old test files
- supporting docs or request notes

If you have more than one source, keep them together. The agent should cross-check them instead of trusting only one export.

Then start the chat with a skill-directed prompt:
...
Follow a phased migration workflow.

Read first:
- package.json
- GLUBEAN.md if present
- tests/, explore/, types/, schemas/, and existing configure/auth helpers

Then read these migration sources:
