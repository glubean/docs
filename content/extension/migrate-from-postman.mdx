...
# Migrate Existing API Assets into Glubean

You do **not** need to rewrite every collection, spec, or legacy suite by hand.
...
<AskAgent
  prompts={[
    {
      intent: "Migrate",
      text: "Use the Glubean skill and migrate «my Postman collection / OpenAPI spec / .http files» — scan the assets, convert one representative pattern for my review, then batch-convert the rest.",
    },
    {
      intent: "Small batch",
      text: "Convert «these curl commands» into contract cases, keeping environment values as {{KEY}} placeholders instead of hardcoded hosts.",
    },
  ]}
/>

For stable API promises, migrate toward contracts and workflows when possible.
Use raw `test()` for migration slices, custom setup, diagnostics, or behavior
that needs unrestricted TypeScript.

The goal is not "Postman JSON becomes one test file." The goal is **one set of
Glubean assets**: executable contracts for durable promises, workflows for
lifecycle behavior, and raw `test()` checks where unrestricted TypeScript is the
right tool.
...
The agent needs to see `package.json`, `@glubean/sdk`, your `contracts/`,
`tests/`, `glubean.yaml`, and any existing config helpers. Without that context
it will default to generic Node or Jest-style output instead of Glubean
patterns.
</Callout>
...
```text
/glubean help me migrate these API assets into Glubean contracts, workflows, and tests
```
...
- it nudges the agent toward contracts/workflows when durable behavior should
  become source of truth, and toward `configure()`, builder-style raw tests,
  types, schemas, and CI-safe structure when raw `test()` is the better fit
...
It is a **verification architecture** task:
...
If you skip those decisions, bulk conversion usually creates a large pile of
weak assets: contracts that do not describe real promises, workflows that do not
prove useful lifecycles, and tests that only check status codes.
...
- whether two requests belong in one contract workflow, one builder-style raw
  test, or separate checks
...
7. run and tighten the migrated assets
...
Source assets almost never describe auth correctly. Postman inherits from folder-level settings, Apifox uses globals, OpenAPI `securitySchemes` rarely match runtime behavior, and `.http` files hardcode expired tokens. Ask the agent to present its auth evidence and proposed strategy, then confirm or correct before it writes contracts, workflows, or tests.
</Callout>
...
```text
/glubean I want to migrate existing API assets into Glubean contracts, workflows, and tests.
...
- **Auth helpers**: whether `@glubean/auth` is needed or plain `configure()` headers are enough
...
- when to use `workflow()` vs builder-style raw tests vs independent checks
...
| secret token | `configure({ secrets: { token: "{{API_TOKEN}}" } })` or auth helper config |
...
| one request block | one `contract.http` case or one `test()` export |
| one durable lifecycle | `workflow()` composed from contract cases |
| one custom runtime sequence with setup and cleanup | builder-style raw `test()` |
...
- use `workflow()` for durable lifecycle promises and builder-style raw tests
  only for custom runtime sequences
- keep durable isolated requests as `contract.http` cases and ad-hoc checks as
  separate `test()` exports
- add status + key field assertions
- mark unclear script behavior with TODO instead of guessing

Return:
1) shared config changes
2) one migrated Glubean asset file
...
- convert setup/cleanup into `workflow()` or builder-style raw tests only when
  state truly carries across steps
- prefer contracts for durable API promises, workflows for reusable lifecycles,
  and raw `test()` for custom runtime behavior
...
Then use the shared client from contracts, workflows, or raw tests:
...
- [ ] durable endpoint behavior became contracts where possible
- [ ] stateful sequences use `workflow()` or builders only when needed
- [ ] write-path assets include reliable cleanup or a documented cleanup policy
