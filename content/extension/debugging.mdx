...
Glubean tests are real Node.js code. You can import any npm package — crypto
libraries, database clients, file parsers, third-party SDKs — and debug them
with the full power of VS Code's debugger. When a crypto function returns an
unexpected hash, or a third-party SDK throws a cryptic error, set a breakpoint
and inspect the actual runtime state.
...
export const createAndFetch = test("create-and-fetch")
  .step("create", async (ctx) => {
    const res = await api.post("api/users", { json: { name: "Alice" } });
    const body = await res.json<{ id: string }>();  // set a breakpoint here
    return { id: body.id };
  })
