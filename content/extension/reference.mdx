...
The run/discovery commands below are centered on raw `test()` files that match
`*.test.ts`, `*.test.js`, or `*.test.mjs`. Use package scripts, MCP, or the CLI
for contract/workflow project runs.
