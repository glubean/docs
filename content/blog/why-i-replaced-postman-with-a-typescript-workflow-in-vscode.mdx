...
<Callout type="info">
  This essay describes the exploration-to-test path that led to Glubean. The current
  product goes further: contract and workflow source can become executable evidence,
  Specifications, docs, reports, and agent context. For the current mental model, start
  with [How Glubean Works](/how-glubean-works).
</Callout>
...
export const verifyAndUpdate = test("verify-and-update")
  .setup(async (ctx) => {
    ...
    return { userId: body.id, originalName: body.firstName };
  })
  .step("Check user profile", async (ctx, { userId, originalName }) => {
    const res = await api.get(`users/${userId}`);
    ctx.expect(res).toHaveStatus(200);
    const body = await res.json<{ firstName: string }>();
    ctx.expect(body.firstName).toBe(originalName);
    return { userId, originalName: body.firstName };
  })
  .step("Update user name", async (ctx, { userId, originalName }) => {
...
```bash
npx glubean run explore/dummyjson/smoke.test.ts --verbose --emit-full-trace
```

The same file can run in your terminal or CI with no export step and no collection sync. `glubean run` writes `.glubean/last-run.result.json`; open it in VS Code and Glubean renders the same trace-oriented view there too.
...
1. Read [How Glubean Works](/how-glubean-works) for the current product model.
2. Follow [Start Here](/getting-started) and use the agent-first setup path.
...
Glubean is open source, and the local workflow is free. The product direction
is simple: keep API exploration in code, promote durable behavior into
contracts and workflows, and make that loop work better with AI instead of
worse.

## Keep reading

<Cards num={2}>
  <Cards.Card title="How Glubean Works" href="/how-glubean-works" arrow>
    See how authored contracts become executable evidence, specs, docs, reports, and agent context.
  </Cards.Card>
  <Cards.Card title="Migrate from Postman / OpenAPI" href="/extension/migrate-from-postman" arrow>
    Use AI to convert existing API assets into Glubean contracts, workflows, or tests.
  </Cards.Card>
</Cards>
