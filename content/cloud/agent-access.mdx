...
This page is for an external or IDE agent that needs read-only Cloud failure
data. It is separate from the built-in [Glubean webapp Agent](/cloud/agent),
which explains the current webapp page directly.
...
Create a scope-minimal token per agent. Cloud webapp → **Tokens** → new
**scoped** token:
...
When your agent runtime supports secret storage or environment variables, use
that instead of pasting the token into chat. If you do paste a token, use a
short-lived scoped token with `runs:read` only and revoke it after the
investigation.
