...
(An `in` mapping returns the case's **logical input**, not HTTP fields — so a
case consumed this way declares `needs: z.object({ id: z.string() })` and maps
it with `pathParams: ({ id }) => ({ id })`, unlike the static
`pathParams: { id: "usr_123" }` variant shown earlier. See
[Workflow](/sdk/contract-flow#basic-workflow) for the full pattern.)
