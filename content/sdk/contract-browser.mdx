...
<AskAgent
  prompts={[
    {
      intent: "Declare a journey",
      text: "Use the Glubean skill and declare «log in and land on the dashboard» as a contract.browser journey, then replay it with glubean run.",
    },
    {
      intent: "Link UI to API",
      text: "Add expect.calls to my «checkout» journey so it asserts the UI actually hit the contract.http case behind it.",
    },
  ]}
/>
...
await installPlugin(browserPlugin); // registers the contract.browser Mode A adapter
```
...
- [Browser Plugin](/sdk/plugins/browser) — the lower-level `browser()` client
  that Mode A drives
