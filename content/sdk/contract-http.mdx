...
| `extensions` | `Record<\`x-${string}\`, unknown>` | OpenAPI `x-*` extensions inherited by all contracts |
...
| `extensions` | `Record<\`x-${string}\`, unknown>` | No | OpenAPI `x-*` extensions at the operation level |
...
| `extensions` | `Record<\`x-${string}\`, unknown>` | No | OpenAPI `x-*` extensions on the case, merged over contract + instance |
