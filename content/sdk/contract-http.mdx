  ...
  prompts={[
    {
      ...
      text: "Use the Glubean skill and write a contract.http spec for «my endpoint» covering the success, validation-error, and auth-failure cases. Run it and show me the results.",
    },
    {
      intent: "From something I have",
      text: "Convert «this OpenAPI operation / curl / Postman request» into a contract case with a Zod response schema.",
    },
    {
      intent: "Tighten",
      text: "Review my contract for «endpoint» and tighten it: pin fields the API actually returns, add a needs schema, and cover the missing error cases.",
    },
  ]}
/>
...
- The first argument is a **case reference** (`getMe.case("authorized")`). The
  overlay's return type is checked against that case's `needs` at compile time,
  and validated again at runtime before the case runs.
- The returned value feeds every function-valued field of the case — `headers`,
  `body`, `pathParams`, `query`. Need a project id in the URL? Return it here and
  read it with `pathParams: ({ projectId }) => ({ projectId })`.
- `ctx.cleanup(fn)` registers teardown that runs in LIFO order after the case.
  `ctx.http`, `ctx.vars`, and `ctx.secrets` are available for setup.
- **One overlay per case.** Registering a second for the same case is a load-time
  error.
...
The file must end in `.bootstrap.ts` — a dot, not a dash. The harness only
...
Use `inboundCase()` for counterparty promises such as webhooks. Inbound cases are projected but not runnable as standalone tests; await them from [`workflow().poll()`](/sdk/contract-flow).
...
glubean contracts --format list-formats        # built-in and plugin formats
```
...
```bash
glubean contracts --projection public-openapi
glubean contracts --projection all
```

When `--projection` is used, each projection carries its own `dir`, `format`, `title`, and `output`; the direct `--dir`, `--format`, and `--title` flags are ignored.
...
- [Workflow](/sdk/contract-flow) — compose contract cases into lifecycles
- [test() API](/sdk/test)
