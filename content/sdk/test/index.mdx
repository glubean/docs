...
<AskAgent
  prompts={[
    {
      intent: "Write a check",
      text: "Use the Glubean skill and write a raw test() for «behavior that doesn't fit a contract», using a shared configure() client. Run it.",
    },
    {
      intent: "Contract or test?",
      text: "Should «this check» be a contract case or a raw test()? Decide, explain the trade-off in one paragraph, and write it.",
    },
  ]}
/>
