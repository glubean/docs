...
<AskAgent
  prompts={[
    {
      intent: "Model a lifecycle",
      text: "Use the Glubean skill and model «register → login → get profile» as a workflow() over my existing contract cases, then run it.",
    },
    {
      intent: "Async flow",
      text: "My «export» flow is async — compose a workflow that requests it, then polls the status case until it is ready, with a sensible bound.",
    },
  ]}
/>
