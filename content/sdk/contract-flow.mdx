...
A workflow composes **existing contract cases**. It does not redeclare endpoints — you reference cases already defined in your `contract.http.with()` specs and wire them together with pure lens functions.
...
`in` and `out` must be pure synchronous lenses — field lookup and repacking only. Do not do I/O or return a Promise from them. Put arbitrary async work in `action()`.

### `compute()`

Pure synchronous state transform. Use it for reshaping, derived fields, or values a lens should not build inline (template literals, `.map()`, method calls).
...
Use `poll()` to repeat a contract case until a declarative or runtime exit predicate holds. Bounds are required: a total timeout and/or max attempts with a per-attempt budget.
