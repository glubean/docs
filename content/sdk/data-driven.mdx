...
3. **`fromDir.merge(path)`** — Treats each file as a **dictionary of named examples**. Merges all dictionaries into one giant object. Perfect for `test.pick()` when named examples are split across files (e.g., `us-east.json`, `eu-west.json`). Combined with the `*.local.json` gitignore convention, this enables [git-safe personal test data](/guides/local-data).
