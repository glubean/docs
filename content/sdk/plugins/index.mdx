...
const { internalApi } = configure({
  plugins: {
    internalApi: myClient({ endpointKey: "INTERNAL_API_URL" }),
  },
});

export const health = test("internal-health", async (ctx) => {
  const status = await internalApi.getStatus();
...
This is what `@glubean/graphql`, `@glubean/grpc`, and `@glubean/browser` are.
`@glubean/auth` is different: it provides HTTP auth helper functions and a
builder transform, so you use it through `configure({ http: ... })` or
`test().use(...)` rather than `installPlugin(...)`.

This is also the openness boundary for contracts. `contract.http` and
`contract.browser` are first-party paths, but protocols are not closed: GraphQL
and gRPC install `contract.graphql` / `contract.grpc`, and a team plugin can
register another protocol the same way.
...
- [Browser (`@glubean/browser`)](/sdk/plugins/browser) — Puppeteer-powered browser automation
- [GraphQL (`@glubean/graphql`)](/sdk/plugins/graphql) — Typed GraphQL queries, mutations, and contract protocol
- [gRPC (`@glubean/grpc`)](/sdk/plugins/grpc) — gRPC status matchers and contract protocol

All of these require `await installPlugin(...)` in `glubean.setup.ts` when you
use their global matchers or contract protocols.

## First-party helper packages

- [Auth (`@glubean/auth`)](/sdk/plugins/auth) — bearer, API key, Basic Auth,
  OAuth helpers, and `withLogin(...)` builder transforms
