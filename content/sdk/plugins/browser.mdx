...
<AskAgent
  prompts={[
    {
      intent: "First browser check",
      text: "Add @glubean/browser and write a check that «opens my app and verifies a page renders», with auto-screenshots and console forwarding on.",
    },
    {
      intent: "Raw test or journey?",
      text: "For «this UI flow», decide between a raw browser test() and a declarative contract.browser journey, and scaffold the one that fits.",
    },
  ]}
/>
...
This page shows the **client factory** path: `browser(...)` inside
`configure({ plugins })`, useful for raw `test()` browser checks. If you are
authoring declarative browser journeys with `contract.browser.with(...)`, also
install the package's **plugin manifest** in `glubean.setup.ts`; see
[Contract Browser](/sdk/contract-browser).
...
const config = configure({
  ...
  plugins: {
    chrome: browser({ launch: true, baseUrl: "appUrl" }),
  },
});
...
`baseUrl` points to a configured vars key (`appUrl` above), so relative
`page.goto("/login")` calls resolve against the runtime `APP_URL` value.
...
export const profileConsistency = browserTest(
  ...
  async (ctx) => {
    ...
    const res = await config.http.get("me");
...
| Shared evidence history | HTML/trace artifacts | Result Viewer + Cloud Targets |
